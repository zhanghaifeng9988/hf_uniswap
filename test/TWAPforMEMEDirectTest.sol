...
contract MockUniswapV2Pair is IUniswapV2Pair {
    ...
    function sync() external override {
        ...
        blockTimestampLast = uint32(block.timestamp); // 最后更新时间戳
    }
...
contract MockUniswapV2Router {
    ...
    // 其他函数可以使用 revert("MockRouter: NOT_IMPLEMENTED")
    receive() external payable {}
}

// 测试合约
contract TWAPforMEMEDirectTest is Test {
    ...
    function setUp() public {
        ...
        // 部署模拟合约
        mockToken1 = new MockERC20("Token1", "TK1"); // WETH
        mockToken2 = new MockERC20("Token2", "TK2");
        mockFactory = new MockUniswapV2Factory(address(mockToken1));
        ...
        // 部署TWAPforMEME合约
        twapOracle = new TWAPforMEME(address(mockFactory), address(mockToken1));
    }
    
    function testMEMEPriceAfterMultipleBuys() public {
        // 添加初始流动性
        uint256 tokenAmount = 10 * 10**18; // 10个代币，与铸造数量匹配
        uint256 ethAmount = 0.01 ether; // 减少ETH数量以保持比例
        uint256 largerEthAmount = 0.1 ether; // 0.1 ETH
        
        // 使用 mintInscription 函数铸造代币
        vm.deal(address(this), 0.001 ether); // 每次铸造需要 0.001 ETH (10 * 0.0001)
        memeInscription.mintInscription{value: 0.001 ether}(tokenAddr);
        
        // 授权路由器使用代币
        MEME_Token(tokenAddr).approve(address(mockRouter), tokenAmount);
        ...
        twapOracle.updatePrice(tokenAddr);
        
        // 第一次购买
        vm.deal(address(this), 0.05 ether);
        memeInscription.buyMeme{value: 0.05 ether}(tokenAddr, 0);
        ...
        uint256 twapPrice = twapOracle.getTWAP(tokenAddr, period);
        
        // 输出或断言价格变化
        console.log("TWAP Price after multiple buys:", twapPrice);
        assertTrue(twapPrice > 0, "TWAP price should be greater than 0");
    }
}